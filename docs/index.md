# Index

Start here:

- 🏠 [README](../README.md)
- 🗺️ [Migration map](migration-map.md)

## Core Areas

- 🛡️ [Blue Team](blue-team/README.md)
- 🧭 [Threat Hunting](threat-hunting/README.md)
- 🔎 [Red Team](red-team/README.md)
- 🧾 [Forensics](forensics/README.md)
- 🧪 [AppSec](appsec/README.md)

## Platforms

- ☁️ [Azure](azure/README.md)
- 🪟 [Windows](windows/README.md)
- 🐧 [Linux](linux/README.md)
- ⌨️ [PowerShell](powershell/README.md)

## Utilities

- 📘 [Playbooks](playbooks/README.md)
- ✂️ [Snippets](snippets/README.md)

## Quick Access

- 🛡️ [Phishing and Triage](blue-team/phishing-triage.md)
- 🧪 [Sandboxing](blue-team/sandboxing.md)
- 🔎 [OSINT and Scanning](red-team/osint-scanning.md)
- 🛠️ [Tools](red-team/tools.md)
- 🧫 [Lab Targets](red-team/lab-targets.md)
- 🪟 [GPO and AD](windows/gpo-ad.md)
- ⌨️ [Active Directory](powershell/active-directory.md)
- ☁️ [Identity and MFA](azure/identity-mfa.md)
- 🧪 [XSS and Conditional Access](appsec/xss-conditional-access.md)
- 🧾 [Investigation Notes](forensics/investigation-notes.md)
- 📘 [Password Policy](playbooks/password-policy.md)
- ✂️ [Nmap Commands](snippets/nmap-commands.md)
- ⚙️ [GitHub and Ops](snippets/github-ops.md)
